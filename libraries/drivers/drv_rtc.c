...
#else
static rt_err_t rt_hw_rtc_register_52832(rt_device_t device, const char *name, rt_uint32_t flag)
{
    struct tm time_new = ONCHIP_RTC_TIME_DEFAULT;
...
#else
    result = rt_hw_rtc_register_52832(&rtc, "rtc", RT_DEVICE_FLAG_RDWR);
#endif
    if (result != RT_EOK) {
