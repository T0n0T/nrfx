...
static int mqtt_connect_with_results(mqtt_client_t *c)
{
    ...
    if (NULL == c) {
        KAWAII_MQTT_LOG_E("%s:%d %s()... mqtt structure is null...", __FILE__, __LINE__, __FUNCTION__);
        RETURN_ERROR(KAWAII_MQTT_NULL_VALUE_ERROR);
    }

    if (CLIENT_STATE_CONNECTED == mqtt_get_client_state(c)) {
        KAWAII_MQTT_LOG_E("%s:%d %s()... mqttclient is already connect...", __FILE__, __LINE__, __FUNCTION__);
        RETURN_ERROR(KAWAII_MQTT_SUCCESS_ERROR);
    }
    ...
    if (KAWAII_MQTT_SUCCESS_ERROR != rc) {
        ...
        KAWAII_MQTT_LOG_E("%s:%d %s()... mqtt connect fail...", __FILE__, __LINE__, __FUNCTION__);
    ...
    if ((len = MQTTSerialize_connect(c->mqtt_write_buf, c->mqtt_write_buf_size, &connect_data)) <= 0) {
        KAWAII_MQTT_LOG_E("%s:%d %s()... mqttclient connectpack serialize fail...", __FILE__, __LINE__, __FUNCTION__);
        goto exit;
    }
    ...
    if ((rc = mqtt_send_packet(c, len, &connect_timer)) != KAWAII_MQTT_SUCCESS_ERROR)
        KAWAII_MQTT_LOG_E("%s:%d %s()... mqttclient send connectpack fail...", __FILE__, __LINE__, __FUNCTION__);
    goto exit;
    ...
    if (mqtt_wait_packet(c, CONNACK, &connect_timer) == CONNACK) {
        if (MQTTDeserialize_connack(&connack_data.session_present, &connack_data.rc, c->mqtt_read_buf, c->mqtt_read_buf_size) == 1) {
            ...
            KAWAII_MQTT_LOG_E("%s:%d %s()... mqttclient connectpack refuse...", __FILE__, __LINE__, __FUNCTION__);
            rc = KAWAII_MQTT_CONNECT_FAILED_ERROR;
        }
    } else {
        KAWAII_MQTT_LOG_E("%s:%d %s()... mqttclient recv connectpack fail...", __FILE__, __LINE__, __FUNCTION__);
        rc = KAWAII_MQTT_CONNECT_FAILED_ERROR;
    }
