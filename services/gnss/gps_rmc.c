...
void UTC2BTC(gps_date_t *date)
{
    ...
    sprintf(
        date->string, GPS_DATE_TO_STRING_FORMAT, date->year, date->month, date->day, date->hour, date->minute, date->second);
}
/**
 * @name:   gps_date_parse
 * @msg:    GPS RMC 时间解析
...
char gps_rmc_parse(gps_info_t info, char *buff)
{
    ...
    printf(
        ...
        info->speed.string, info->direction.string, info->ADEN);
#endif // GPS_RMC_SHOW_DEBUG
    return 1;
}
