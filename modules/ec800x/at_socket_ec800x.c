...
#define EC800X_EVENT_CONN_OK   (1L << 0)
#define EC800X_EVENT_SEND_OK   (1L << 1)
#define EC800X_EVENT_RECV_OK   (1L << 2)
#define EC800X_EVNET_CLOSE_OK  (1L << 3)
#define EC800X_EVENT_CONN_FAIL (1L << 4)
#define EC800X_EVENT_SEND_FAIL (1L << 5)
#define EC800X_EVENT_DOMAIN_OK (1L << 6)
...
static int ec800x_socket_event_send(struct at_device *device, uint32_t event)
{
    return (int)rt_event_send(device->socket_event, event);
}

static int ec800x_socket_event_recv(struct at_device *device, uint32_t event, uint32_t timeout, rt_uint8_t option)
{
    int result = RT_EOK;
    rt_uint32_t recved;
 ...
 *         -5: no memory
 */
static int ec800x_socket_close(struct at_socket *socket)
{
    int result               = RT_EOK;
 ...
 *          -5: no memory
 */
static int ec800x_socket_connect(struct at_socket *socket, char *ip, int32_t port,
...
#define CONN_RETRY 2
    ...
    for (i = 0; i < CONN_RETRY; i++) {
        ...
        if (ec800x_socket_event_recv(device, SET_EVENT(device_socket, 0),
        ...
        event_result = ec800x_socket_event_recv(device, EC800X_EVENT_CONN_OK | EC800X_EVENT_CONN_FAIL,
        ...
        /* check result */
        if (event_result & EC800X_EVENT_CONN_OK) {
            result = RT_EOK;
            break;
        }
        ...
        if (ec800x_socket_close(socket) < 0) {
            result = -RT_ERROR;
            break;
        }
    }
 ...
 *          -5: no memory
 */
static int ec800x_socket_send(struct at_socket *socket, const char *buff, size_t bfsz, enum at_socket_type type)
{
    uint32_t event = 0;
    int result = 0, event_result = 0;
    ...
    ec800x->user_data = (void *)device_socket;

    /* clear socket send event */
    event = SET_EVENT(device_socket, EC800X_EVENT_SEND_OK | EC800X_EVENT_SEND_FAIL);
    ec800x_socket_event_recv(device, event, 0, RT_EVENT_FLAG_OR);
    ...
    while (sent_size < bfsz) {
        if (bfsz - sent_size < EC800X_MODULE_SEND_MAX_SIZE) {
            cur_pkt_size = bfsz - sent_size;
        } else {
            cur_pkt_size = EC800X_MODULE_SEND_MAX_SIZE;
        }
        ...
        event_result = ec800x_socket_event_recv(device, event, 10 * RT_TICK_PER_SECOND, RT_EVENT_FLAG_OR);
        if (event_result < 0) {
        ...
        /* waiting OK or failed result */
        event        = EC800X_EVENT_SEND_OK | EC800X_EVENT_SEND_FAIL;
        event_result = ec800x_socket_event_recv(device, event, 1 * RT_TICK_PER_SECOND, RT_EVENT_FLAG_OR);
        if (event_result < 0) {
        ...
        /* check result */
        if (event_result & EC800X_EVENT_SEND_FAIL) {
 ...
 *         -5: no memory
 */
static int ec800x_domain_resolve(const char *name, char ip[16])
{
#define RESOLVE_RETRY 3
    ...
    /* clear EC800X_EVENT_DOMAIN_OK */
    ec800x_socket_event_recv(device, EC800X_EVENT_DOMAIN_OK, 0, RT_EVENT_FLAG_OR);

    ec800x              = (struct at_device_ec800x *)device->user_data;
    ec800x->socket_data = ip;
    ...
    for (i = 0; i < RESOLVE_RETRY; i++) {
        ...
        if (ec800x_socket_event_recv(device, EC800X_EVENT_DOMAIN_OK, 10 * RT_TICK_PER_SECOND, RT_EVENT_FLAG_OR) < 0) {
            result = -RT_ETIMEOUT;
            continue;
        } else {
            if (rt_strlen(ip) < 8) {
 ...
 */
static void ec800x_socket_set_event_cb(at_socket_evt_t event, at_evt_cb_t cb)
{
...
static void urc_send_func(struct at_client *client, const char *data, rt_size_t size)
{
    ...
    ec800x        = (struct at_device_ec800x *)device->user_data;
    device_socket = (int)ec800x->user_data;

    if (rt_strstr(data, "SEND OK")) {
        ec800x_socket_event_send(device, SET_EVENT(device_socket, EC800X_EVENT_SEND_OK));
    } else if (rt_strstr(data, "SEND FAIL")) {
        ec800x_socket_event_send(device, SET_EVENT(device_socket, EC800X_EVENT_SEND_FAIL));
    }
}
...
static void urc_dnsqip_func(struct at_client *client, const char *data, rt_size_t size)
{
    ...
    ec800x = (struct at_device_ec800x *)device->user_data;
    if (ec800x->socket_data == RT_NULL) {
        LOG_D("%s device socket_data no config.", ec800x->device_name);
        return;
    }
    ...
    if (j == 3) {
        ...
        rt_memcpy(ec800x->socket_data, recv_ip, sizeof(recv_ip));

        ec800x_socket_event_send(device, EC800X_EVENT_DOMAIN_OK);
    } else {
...
static void urc_qiurc_func(struct at_client *client, const char *data, rt_size_t size)
{
    ...
    switch (*(data + 9)) {
        ...
        case 'r':
            ...
            break; //+QIURC: "recv"
        case 'p':
            urc_pdpdeact_func(client, data, size);
...
static const struct at_socket_ops ec800x_socket_ops =
    {
        ec800x_socket_connect,
        ec800x_socket_close,
        ec800x_socket_send,
        ec800x_domain_resolve,
        ec800x_socket_set_event_cb,
...
int ec800x_socket_class_register(struct at_device_class *class)
{
    ...
    class->socket_num = AT_DEVICE_EC800X_SOCKETS_NUM;
    class->socket_ops = &ec800x_socket_ops;

    return RT_EOK;
}

#endif /* AT_DEVICE_USING_EC800X && AT_USING_SOCKET */
